...
let normalizeDuration$1 = new Helpers().normalizeDuration;
...
const _PersianDateClass = class _PersianDateClass {
  /**
   * @param input
  ...
  constructor(input) {
     ...
     * @returns {Duration}
     */
    __publicField(this, "duration", function(input, key) {
     ...
     * @returns {*}
     */
    __publicField(this, "utc", function(input) {
      let utcStamp;
      if (input) {
    ...
    this.calendarType = _PersianDateClass.calendarType;
    this.localType = _PersianDateClass.localType;
    this.leapYearMode = _PersianDateClass.leapYearMode;
  ...
  setup(input) {
    ...
    } else if (input instanceof _PersianDateClass) {
      this.algorithmsCalc([
        input.year(),
        input.month(),
        input.date(),
  ...
  _getSyncedClass(input) {
    let syncedCelander = _PersianDateClass.toCalendar(this.calendarType).toLocale(this.localType).toLeapYearMode(this.leapYearMode);
   ...
   */
  static rangeName() {
    const p = _PersianDateClass, t = p.calendarType;
    if (p.localType === "fa") {
      if (t === "persian") {
        return fa.persian;
      } else {
  ...
  static isPersianDate(obj) {
    return obj instanceof _PersianDateClass;
  }
  /**
   * @param obj
   * @return {boolean}
  ...
  static getFirstWeekDayOfMonth(year, month) {
    return new _PersianDateClass([year, month, 1]).day();
  }
  /**
   * @param year
   * @param month
   ...
   */
  startOf(key) {
    let syncedCelander = _PersianDateClass.toCalendar(this.calendarType).toLocale(this.localType);
    let newArray = new _PersianDateClass(this.valueOf() - (this.calendar().weekday - 1) * 864e5).toArray();
    switch (key) {
      case "years":
      case "year":
  ...
  local() {
    ...
    if (this._utcMode) {
      ...
      this.toCalendar(_PersianDateClass.calendarType);
  ...
  add(key, value) {
    ...
    if (unit === "year") {
      ...
      let tempDate = new _PersianDateClass([arr[0] + value, arr[1], normalizedDate, arr[3], arr[4], arr[5], arr[6], arr[7]]);
      return tempDate;
    }
    if (unit === "month") {
      ...
      let normalizaedDate = arr[2], tempDateArray = new _PersianDateClass([arr[0] + tempYear, calcedMonth, 1, arr[3], arr[4], arr[5], arr[6], arr[7]]).toArray(), monthDays = this.daysInMonth(arr[0] + tempYear, calcedMonth);
      if (arr[2] > monthDays) {
        normalizaedDate = monthDays;
      }
      return new _PersianDateClass([tempDateArray[0], tempDateArray[1], normalizaedDate, tempDateArray[3], tempDateArray[4], tempDateArray[5], tempDateArray[6], tempDateArray[7]]);
    }
    if (unit === "day") {
      let calcedDay = new _PersianDateClass(this.valueOf()).hour(12), newMillisecond = calcedDay.valueOf() + value * 864e5, newDate = new _PersianDateClass(newMillisecond);
      return newDate.hour(arr[3]);
    }
    if (unit === "week") {
      let calcedDay = new _PersianDateClass(this.valueOf()).hour(12), newMillisecond = calcedDay.valueOf() + 7 * value * 864e5, newDate = new _PersianDateClass(newMillisecond);
      return newDate.hour(arr[3]);
    }
    if (unit === "hour") {
 ...
 * @return {*}
 */
__publicField(_PersianDateClass, "utc", function(input) {
  if (input) {
    return new _PersianDateClass(input).utc();
  } else {
    return new _PersianDateClass().utc();
  }
});
let PersianDateClass = _PersianDateClass;
PersianDateClass.calendarType = "persian";
PersianDateClass.leapYearMode = "astronomical";
PersianDateClass.localType = "fa";
export {
  PersianDateClass as default
};
