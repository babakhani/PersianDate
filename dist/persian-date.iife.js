...
//#region src/astro.js
	var ASTRO = class {
		...
		*/
		obliqeq(jd) {
			var eps, u, v = u = (jd - this.J2000) / (this.JulianCentury * 100), i;
			eps = 23.43929111111111;
			if (Math.abs(u) < 1) for (i = 0; i < 10; i++) {
		...
		*/
		nutation(jd) {
			...
			t3 = t * (t2 = t * t);
			ta[0] = this.dtr(297.850363 + 445267.11148 * t - .0019142 * t2 + t3 / 189474);
			ta[1] = this.dtr(357.52772 + 35999.05034 * t - 1603e-7 * t2 - t3 / 3e5);
			ta[2] = this.dtr(134.96298 + 477198.867398 * t + .0086972 * t2 + t3 / 56250);
			ta[3] = this.dtr(93.27191 + 483202.017538 * t - .0036825 * t2 + t3 / 327270);
			ta[4] = this.dtr(125.04452 - 1934.136261 * t + .0020708 * t2 + t3 / 45e4);
			for (i = 0; i < 5; i++) ta[i] = this.fixangr(ta[i]);
			to10 = t / 10;
			for (i = 0; i < 63; i++) {
				ang = 0;
				for (j = 0; j < 5; j++) if (this.nutArgMult[i * 5 + j] !== 0) ang += this.nutArgMult[i * 5 + j] * ta[j];
		...
		deltat(year) {
			...
			if (year >= 1620 && year <= 2e3) {
				...
				if (year < 948) dt = 2177 + 497 * t + 44.1 * t * t;
					...
					if (year > 2e3 && year < 2100) dt += .37 * (year - 2100);
				}
			}
			return dt;
		}
		...
		*/
		equinox(year, which) {
			...
			W = 35999.373 * T - 2.47;
			deltaL = 1 + .0334 * this.dcos(W) + 7e-4 * this.dcos(2 * W);
			S = 0;
			for (i = j = 0; i < 24; i++) {
		...
		*/
		sunpos(jd) {
			...
			M = this.fixangle(M);
			e = .016708634 + -42037e-9 * T + -1.267e-7 * T2;
			C = (1.914602 + -.004817 * T + -14e-6 * T2) * this.dsin(M) + (.019993 - 101e-6 * T) * this.dsin(2 * M) + 289e-6 * this.dsin(3 * M);
			sunLong = L0 + C;
			sunAnomaly = M + C;
			sunR = 1.000001018 * (1 - e * e) / (1 + e * this.dcos(sunAnomaly));
			Omega = 125.04 - 1934.136 * T;
			Lambda = sunLong + -.00569 + -.00478 * this.dsin(Omega);
			epsilon0 = this.obliqeq(jd);
			epsilon = epsilon0 + .00256 * this.dcos(Omega);
		...
		*/
		equationOfTime(jd) {
			...
			L0 = 280.4664567 + 360007.6982779 * tau + .03032028 * tau * tau + tau * tau * tau / 49931 + -(tau * tau * tau * tau / 15300) + -(tau * tau * tau * tau * tau / 2e6);
			L0 = this.fixangle(L0);
			alpha = this.sunpos(jd)[10];
			deltaPsi = this.nutation(jd)[0];
			epsilon = this.obliqeq(jd) + this.nutation(jd)[1];
			E = L0 + -.0057183 + -alpha + deltaPsi * this.dcos(epsilon);
	...
	var Algorithms = class {
		...
		*/
		tehran_equinox(year) {
			...
			equJD = equJED - this.ASTRO.deltat(year) / (1440 * 60);
		...
		*/
		gWeekDayToPersian(weekday) {
			if (weekday + 2 === 8) return 1;
			else if (weekday + 2 === 7) return 7;
		...
		*/
		updateFromGregorian() {
			...
			if (this.parent._utcMode === false) this.State.zone = this.State.gDate.getTimezoneOffset();
			...
			j = this.gregorian_to_jd(year, mon + 1, mday) + Math.floor(sec + 60 * (min + 60 * hour) + .5) / 86400;
			...
			if (this.State.gregserial.day !== null) this.State.gregserial.day = j - this.J0000;
			utime = (j - this.J1970) * (3600 * 24 * 1e3);
		...
		*/
		calcGregorian(dateArray) {
			if (dateArray[0] || dateArray[0] === 0) this.State.gregorian.year = dateArray[0];
			if (dateArray[1] || dateArray[1] === 0) this.State.gregorian.month = dateArray[1];
			if (dateArray[2] || dateArray[2] === 0) this.State.gregorian.day = dateArray[2];
			if (dateArray[3] || dateArray[3] === 0) this.State.gregorian.hour = dateArray[3];
			if (dateArray[4] || dateArray[4] === 0) this.State.gregorian.minute = dateArray[4];
			if (dateArray[5] || dateArray[5] === 0) this.State.gregorian.second = dateArray[5];
			if (dateArray[6] || dateArray[6] === 0) this.State.gregorian.millisecond = dateArray[6];
		...
		*/
		calcPersian(dateArray) {
			if (dateArray[0] || dateArray[0] === 0) this.State.persian.year = dateArray[0];
			if (dateArray[1] || dateArray[1] === 0) this.State.persian.month = dateArray[1];
			if (dateArray[2] || dateArray[2] === 0) this.State.persian.day = dateArray[2];
			if (dateArray[3] || dateArray[3] === 0) this.State.gregorian.hour = dateArray[3];
			if (dateArray[4] || dateArray[4] === 0) this.State.gregorian.minute = dateArray[4];
			if (dateArray[5] || dateArray[5] === 0) this.State.gregorian.second = dateArray[5];
			if (dateArray[6] || dateArray[6] === 0) this.State.gregorian.millisecond = dateArray[6];
		...
		*/
		calcPersiana(dateArray) {
			if (dateArray[0] || dateArray[0] === 0) this.State.persianAstro.year = dateArray[0];
			if (dateArray[1] || dateArray[1] === 0) this.State.persianAstro.month = dateArray[1];
			if (dateArray[2] || dateArray[2] === 0) this.State.persianAstro.day = dateArray[2];
			if (dateArray[3] || dateArray[3] === 0) this.State.gregorian.hour = dateArray[3];
			if (dateArray[4] || dateArray[4] === 0) this.State.gregorian.minute = dateArray[4];
			if (dateArray[5] || dateArray[5] === 0) this.State.gregorian.second = dateArray[5];
			if (dateArray[6] || dateArray[6] === 0) this.State.gregorian.millisecond = dateArray[6];
	...
	var Helpers = class {
		...
		*/
		toPersianDigit(digit, latinDigit = false) {
			return digit.toString().replace(/\d+/g, function(digit) {
				...
				for (i = 0; i < digit.length; i += 1) enDigitArr.push(digit.charCodeAt(i));
		...
		*/
		normalizeDuration() {
			...
			if (durationUnit.year.indexOf(unit) > -1) unit = "year";
			else if (durationUnit.month.indexOf(unit) > -1) unit = "month";
			else if (durationUnit.week.indexOf(unit) > -1) unit = "week";
			else if (durationUnit.day.indexOf(unit) > -1) unit = "day";
			else if (durationUnit.hour.indexOf(unit) > -1) unit = "hour";
			else if (durationUnit.minute.indexOf(unit) > -1) unit = "minute";
			else if (durationUnit.second.indexOf(unit) > -1) unit = "second";
			else if (durationUnit.millisecond.indexOf(unit) > -1) unit = "millisecond";
			return {
				unit,
				value
			};
...
//#endregion
//#region src/duration.js
	var normalizeDuration$1 = new Helpers().normalizeDuration;
	...
	*/
	var Duration = class {
		constructor(key, value) {
			let duration = {}, data = this._data = {}, milliseconds = 0, normalizedUnit = normalizeDuration$1(key, value), unit = normalizedUnit.unit;
			duration[unit] = normalizedUnit.value;
			milliseconds = duration.milliseconds || duration.millisecond || duration.ms || 0;
			let years = duration.years || duration.year || duration.y || 0, months = duration.months || duration.month || duration.M || 0, weeks = duration.weeks || duration.w || duration.week || 0, days = duration.days || duration.d || duration.day || 0, hours = duration.hours || duration.hour || duration.h || 0, minutes = duration.minutes || duration.minute || duration.m || 0, seconds = duration.seconds || duration.second || duration.s || 0;
...
validateInputArray(input) {
		let out = true;
		if (input[1] < 1 || input[1] > 12) out = false;
		if (input[2] < 1 || input[1] > 31) out = false;
		if (input[3] < 0 || input[3] > 24) out = false;
		if (input[4] < 0 || input[4] > 60) out = false;
		if (input[5] < 0 || input[5] > 60) out = false;
		return out;
	} };
...
/**
	...
	*/
	var _PersianDateClass;
	var { toPersianDigit, leftZeroFill, normalizeDuration } = new Helpers();
	/**
	* @description persian date class
	*/
	var PersianDateClass = class PersianDateClass {
		/**
		* @param input
		...
		*/
		constructor(input) {
			...
			this.setup(input);
			if (this.State.isInvalidDate) return new Date([-1, -1]);
			return this;
		}
		/**
		* @param input
		*/
		setup(input) {
			if (type_checking_default.isDate(input)) this._gDateToCalculators(input);
			else if (type_checking_default.isArray(input)) {
				if (!validator_default.validateInputArray(input)) {
			...
			} else if (type_checking_default.isNumber(input)) {
				const fromUnix = new Date(input);
				this._gDateToCalculators(fromUnix);
			} else if (input instanceof PersianDateClass) this.algorithmsCalc([
				input.year(),
				input.month(),
				input.date(),
				input.hour(),
		...
		*/
		_getSyncedClass(input) {
			return new (PersianDateClass.toCalendar(this.calendarType).toLocale(this.localType).toLeapYearMode(this.leapYearMode))(input);
		}
		/**
		* @param inputgDate
		* @private
		...
		*/
		static rangeName() {
			...
			if (p.localType === "fa") if (t === "persian") return fa_default.persian;
			else return fa_default.gregorian;
			else if (t === "persian") return en_default.persian;
			else return en_default.gregorian;
		}
		/**
		* @since 1.0.0
		* @description Helper method that return date range name like week days name, month names, month days names (specially in persian calendar).
		...
		*/
		rangeName() {
			...
			if (this.localType === "fa") if (t === "persian") return fa_default.persian;
			else return fa_default.gregorian;
			else if (t === "persian") return en_default.persian;
			else return en_default.gregorian;
		}
		...
		*/
		toLeapYearMode(input) {
			...
			if (input === "astronomical" && this.calendarType == "persian") this.leapYearMode = "astronomical";
			else if (input === "algorithmic" && this.calendarType == "persian") this.leapYearMode = "algorithmic";
		...
		*/
		static toLocale(input) {
			let d = PersianDateClass;
			d.localType = input;
			if (d.localType !== "fa") d.formatPersian = false;
			else d.formatPersian = "_default";
			return d;
		}
		/**
		* @since 1.0.0
		...
		*/
		algorithmsCalc(dateArray) {
			if (this.isPersianDate(dateArray)) dateArray = [
				dateArray.year(),
				dateArray.month(),
				dateArray.date(),
				dateArray.hour(),
			...
			if (this.calendarType === "persian" && this.leapYearMode == "algorithmic") return this.algorithms.calcPersian(dateArray);
			else if (this.calendarType === "persian" && this.leapYearMode == "astronomical") return this.algorithms.calcPersiana(dateArray);
		...
		*/
		calendar() {
			...
			if (this.calendarType == "persian") {
				if (this.leapYearMode == "astronomical") key = "persianAstro";
				else if (this.leapYearMode == "algorithmic") key = "persianAlgo";
			} else key = "gregorian";
		...
		*/
		year(input) {
			if (input || input === 0) {
				...
				return this;
			} else return this.calendar().year;
		}
		/**
		* @param input
		* @returns {*}
		*/
		month(input) {
			if (input || input === 0) {
				...
				return this;
			} else return this.calendar().month + 1;
		}
		/**
		* Day of week
		...
		*/
		hours(input) {
			if (input || input === 0) {
				...
				return this;
			} else return this.State.gDate.getHours();
		}
		/**
		* @param input
		* @returns {*}
		...
		*/
		minutes(input) {
			if (input || input === 0) {
				...
				return this;
			} else return this.State.gDate.getMinutes();
		}
		/**
		* @param input
		* @returns {*}
		...
		*/
		seconds(input) {
			if (input || input === 0) {
				...
				return this;
			} else return this.State.gDate.getSeconds();
		}
		/**
		* @param input
		* @returns {*}
		...
		*/
		milliseconds(input) {
			if (input || input === 0) {
				...
				return this;
			} else return this.State.gregorian.millisecond;
		}
		/**
		...
		*/
		unix(timestamp) {
			let output;
			if (timestamp) return this._getSyncedClass(timestamp * 1e3);
			else {
		...
		*/
		diff(input, val, asFloat) {
			let self = this, inputMoment = input, diff = self.State.gDate - inputMoment.toDate() - 0, year = self.year() - inputMoment.year(), month = self.month() - inputMoment.month(), date = (self.date() - inputMoment.date()) * -1, output;
			if (val === "months" || val === "month") output = year * 12 + month + date / 30;
			else if (val === "years" || val === "year") output = year + (month + date / 30) / 12;
			else output = val === "seconds" || val === "second" ? diff / 1e3 : val === "minutes" || val === "minute" ? diff / 6e4 : val === "hours" || val === "hour" ? diff / 36e5 : val === "days" || val === "day" ? diff / 864e5 : val === "weeks" || val === "week" ? diff / 6048e5 : diff;
		...
		*/
		startOf(key) {
			...
			switch (key) {
				case "years":
				case "year": return new syncedCelander([
					this.year(),
					1,
					1
				]);
				case "months":
				case "month": return new syncedCelander([
					this.year(),
					this.month(),
					1
				]);
				case "days":
				case "day": return new syncedCelander([
					this.year(),
					this.month(),
					this.date(),
					0,
				...
				case "hours":
				case "hour": return new syncedCelander([
					this.year(),
					this.month(),
					this.date(),
					this.hours(),
				...
				case "minutes":
				case "minute": return new syncedCelander([
					this.year(),
					this.month(),
					this.date(),
					this.hours(),
				...
				case "seconds":
				case "second": return new syncedCelander([
					this.year(),
					this.month(),
					this.date(),
					this.hours(),
				...
				case "weeks":
				case "week": return new syncedCelander(newArray);
		...
		*/
		local() {
			...
			if (this._utcMode) {
				...
				if (ThatDayOffset < 0) utcStamp = this.valueOf() - offsetMils;
				else
 /* istanbul ignore next */
		...
		*/
		utc(input) {
			let utcStamp;
			if (input) return this._getSyncedClass(input).utc();
			if (this._utcMode) return this;
			else {
				let offsetMils = this.zone() * 60 * 1e3;
				if (this.zone() < 0) utcStamp = this.valueOf() + offsetMils;
				else
 /* istanbul ignore next */
				utcStamp = this.valueOf() - offsetMils;
		...
		*/
		isDST() {
			...
			if (month == 1 && day > 1 || month == 6 && day < 31 || month < 6 && month >= 2) return true;
			else return false;
		}
		/**
		* @returns {boolean}
		*/
		isLeapYear(year) {
			if (year === void 0) year = this.year();
			if (this.calendarType == "persian" && this.leapYearMode === "algorithmic") return this.algorithms.leap_persian(year);
			if (this.calendarType == "persian" && this.leapYearMode === "astronomical") return this.algorithms.leap_persiana(year);
			else if (this.calendarType == "gregorian") return this.algorithms.leap_gregorian(year);
		}
		/**
		* @param yearInput
		* @param monthInput
		...
		*/
		daysInMonth(yearInput, monthInput) {
			...
			if (this.calendarType === "gregorian") return new Date(year, month, 0).getDate();
		}
		/**
		...
		*/
		formatNumber() {
			...
			if (this.formatPersian === "_default") if (typeof module !== "undefined" && typeof module.exports !== "undefined")
 /* istanbul ignore next */
			if (self.formatPersian === false) output = false;
			else output = true;
			else if (window.formatPersian === false) output = false;
			else output = true;
			else if (this.formatPersian === true) output = true;
			else if (this.formatPersian === false) output = false;
			return output;
		}
		/**
		* @param inputString
		...
		*/
		format(inputString) {
			if (this.State.isInvalidDate) return false;
			...
			let checkPersian = function(i) {
				if (formatToPersian) return toPersianDigit(i);
				else return i;
			};
			function replaceFunction(input) {
				switch (input) {
					case "a": if (formatToPersian) return info.hour >= 12 ? "ب ظ" : "ق ظ";
					...
					case "w": {
						let t = self.startOf("year");
						return checkPersian(parseInt(self.diff(t, "days") / 7) + 1);
					}
					case "ww": {
						let t = self.startOf("year");
						return checkPersian(leftZeroFill(parseInt(self.diff(t, "days") / 7) + 1, 2));
					}
					...
					case "Z": {
						...
						return checkPersian(flag + leftZeroFill(hours, 2) + ":" + leftZeroFill(minutes, 2));
					}
					/* istanbul ignore next */
					case "ZZ": {
						let flag = "+", hours = Math.round(info.timezone / 60), minutes = info.timezone % 60;
			...
			else return "YYYY-MM-DD HH:mm:ss a".replace(formattingTokens, replaceFunction);
		}
		/**
		* @param key
		* @param value
		...
		*/
		add(key, value) {
			...
			if (unit === "year") {
				...
				if (arr[2] > monthDays) normalizedDate = monthDays;
				return new PersianDateClass([
					arr[0] + value,
					arr[1],
					normalizedDate,
			...
			if (unit === "month") {
				...
				} else calcedMonth = arr[1] + remainingMonth;
				let normalizaedDate = arr[2], tempDateArray = new PersianDateClass([
					arr[0] + tempYear,
					calcedMonth,
					1,
					arr[3],
					...
					arr[6],
					arr[7]
				]).toArray(), monthDays = this.daysInMonth(arr[0] + tempYear, calcedMonth);
				if (arr[2] > monthDays) normalizaedDate = monthDays;
				return new PersianDateClass([
					tempDateArray[0],
					tempDateArray[1],
					normalizaedDate,
			...
			if (unit === "day") return new PersianDateClass(new PersianDateClass(this.valueOf()).hour(12).valueOf() + value * 864e5).hour(arr[3]);
			if (unit === "week") return new PersianDateClass(new PersianDateClass(this.valueOf()).hour(12).valueOf() + 7 * value * 864e5).hour(arr[3]);
			if (unit === "hour") {
	...
	_PersianDateClass = PersianDateClass;
	_defineProperty(PersianDateClass, "duration", function(input, key) {
		return new Duration(input, key);
	});
	_defineProperty(PersianDateClass, "unix", function(timestamp) {
		if (timestamp) return new _PersianDateClass(timestamp * 1e3);
		else return new _PersianDateClass().unix();
	});
	_defineProperty(PersianDateClass, "utc", function(input) {
		if (input) return new _PersianDateClass(input).utc();
		else return new _PersianDateClass().utc();
	});
...
//# sourceMappingURL=persian-date.iife.js.map