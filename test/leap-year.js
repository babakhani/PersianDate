...
describe(`Check total leap years in next ${N_NEXT_YEARS} year, different algorithms`, () => {
    // https://eclipse.gsfc.nasa.gov/SKYCAL/algorithm.js
    const isLeapYearWithNasaAlgorithm = (y) => ((((y - (y > 0 ? 474 : 473)) % 2820) + 474 + 38) * 682) % 2816 < 682;
    ...
    const isLeapYearBirashk = (year) =>
        parseFloat(`0.${((year + 2346) * 0.24219858156).toString().split('.')[1]}`) < 0.24219858156;

    // https://fa.wikipedia.org/wiki/%D8%B3%D8%A7%D9%84_%DA%A9%D8%A8%DB%8C%D8%B3%D9%87
    const isLeapYearwikiPython = (year) => {
        const a = 0.025,
            b = 266;
        if (year > 0) {
        ...
        frac0 = parseInt((leapDays0 - parseInt(leapDays0, 10)) * 1000, 10);
        frac1 = parseInt((leapDays1 - parseInt(leapDays1, 10)) * 1000, 10);
        if (frac0 <= b && frac1 > b) {
            return true;
        } else {
            return false;
