...
describe("Create Instance", function () {
	it("From Array [2017, 1, 12, 1, 1, 1]", function () {
		...
		pDate.toLocale("en");
		assert.deepEqual(
			new pDate([2017, 2, 29, 1, 1, 1]).toArray(),
			[2017, 3, 1, 1, 1, 1, 0],
		);
		assert.deepEqual(
			new pDate([2020, 2, 29, 1, 1, 1]).toArray(),
			[2020, 2, 29, 1, 1, 1, 0],
		);
		assert.deepEqual(
			new pDate([5000, 2, 29, 1, 1, 1]).toArray(),
			[5000, 3, 1, 1, 1, 1, 0],
		);
...
describe("format", function () {
	it("llll", function () {
		...
		pDate.toLocale("en");
		assert.deepEqual(
			new pDate(defaultArray).format(),
			"2017-01-01 12:32:40 PM",
		);
		assert.deepEqual(
			new pDate(defaultArray).format("lllll"),
			"Sun 1 Jan 2017  12:32  PM2017/1/1",
		);
		assert.deepEqual(
			new pDate(defaultArray).format("llll"),
			"Sun 1 Jan 2017  12:32  PM",
		);
		assert.deepEqual(
			new pDate(defaultArray).format("lll"),
			"Jan 2017 01   12:32  PM",
		);
