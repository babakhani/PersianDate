...
describe("Invalid Date", function () {
	it("", function () {
		assert.equal(
			new pDate([1398, -1]).toString(),
			new Date([2019, -1]).toString(),
		);
	});
});
...
describe("toCalendar gregorian", function () {
	...
	it('[1403,1,1] gregorian fa format("dddd")', function () {
		let persainAlgo = new pDate([1404, 1, 1])
				.toCalendar("persian")
			...
			persianAstroWeekday = new pDate([1404, 1, 1])
				...
				.toLocale("en")
				.format("dddd"),
			gregorianWeekday = new pDate([1404, 1, 1])
				.toCalendar("gregorian")
				.toLocale("en")
				.format("dddd");
...
describe("toCalendar persian", function () {
	...
	it('[1403,1,1] persian algorithmic fa format("dddd")', function () {
		let persainAlgoWeekday = new pDate(defArray)
				.toCalendar("persian")
				.toLeapYearMode("algorithmic")
				.toLocale("fa")
				.format("dddd"),
			persianAstroWeekday = new pDate(defArray)
				...
				.toLocale("fa")
				.format("dddd"),
			gregorianWeekday = new pDate(defArray)
				.toCalendar("gregorian")
				.toLocale("fa")
				.format("dddd");
...
describe("rangeName", function () {
	...
	it("weekdaysShort persian fa", function () {
		pDate.toCalendar("persian").toLocale("fa");
		assert.deepEqual(pDate.rangeName().weekdaysShort, [
			"ش",
			"ی",
			"د",
			"س",
	...
	it("weekdaysMin gregorian en", function () {
		pDate.toCalendar("gregorian").toLocale("en");
		assert.deepEqual(new pDate().rangeName().weekdaysMin, [
			"Su",
			"Mo",
			"Tu",
			"We",
	...
	it("weekdaysShort gregorian en", function () {
		pDate.toCalendar("gregorian").toLocale("en");
		assert.deepEqual(new pDate().rangeName().weekdaysShort, [
			"Sun",
			"Mon",
			"Tue",
			"Wed",
...
describe("Millisecond", function () {
	it("get", function () {
		let formattedDate = new pDate([
			1391, 12, 23, 22, 45, 48, 987,
		]).millisecond();
...
describe("format", function () {
	...
	it('format("LLLL")', function () {
		...
		assert.deepEqual(
			...
			"سه شنبه ۱ فروردین ۱۳۹۱  ۱:۰۱  ق ظ",
		);
	});
